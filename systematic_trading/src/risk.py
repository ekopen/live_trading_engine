...
def check_position_size(client, symbol, qty, direction, price, strategy_name, max_allocation=MAX_ALLOCATION):
    try:
        ...
        return False, f"Error in position size check."
...
def check_cash_balance(client, qty, direction, price, strategy_name, symbol):
    try:
        ...
        return False, f"Error in cash balance check."
    return True, f"Cash balance ok."
...
def run_risk_checks(client, symbol, qty, direction, model_price, strategy_name):
    try:
        ...
        logger.exception(f"Error running risk checks for {strategy_name} - {symbol}: {e}")
        return False, [f"Error in risk checks for {strategy_name} - {symbol}"]
